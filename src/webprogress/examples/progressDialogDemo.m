...
%[text] This script demonstrates the two dialog-based progress displays of `webprogress.download`:
...
%[text] Without a `Figure` argument, `webprogress.download` uses a standard waitbar dialog.
deleteIfExists(standardWaitbarFile)
webprogress.download(standardWaitbarFile, demoUrl, ...
    'DisplayMode', 'Dialog Box', ...
    'UpdateInterval', 0.25, ...
    'ShowFilename', true);
%%
%[text] ## Progress Dialog in a UIFigure
%[text] Passing a `uifigure` through the `Figure` argument makes `webprogress.download` use a `uiprogressdlg` instead of a standard waitbar.
...
webprogress.download(uiProgressDialogFile, demoUrl, ...
    'DisplayMode', 'Dialog Box', ...
    'UpdateInterval', 0.25, ...
    'ShowFilename', true, ...
    'Figure', fig);
